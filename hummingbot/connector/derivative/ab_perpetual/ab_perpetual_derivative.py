...
import hummingbot.connector.derivative.ab_perpetual.ab_perpetual_constants as CONSTANTS
from hummingbot.connector.constants import s_decimal_0, s_decimal_NaN
from hummingbot.connector.derivative.ab_perpetual import ab_perpetual_web_utils as web_utils
from hummingbot.connector.derivative.ab_perpetual.ab_perpetual_api_order_book_data_source import (
    AbPerpetualAPIOrderBookDataSource,
)
from hummingbot.connector.derivative.ab_perpetual.ab_perpetual_user_stream_data_source import (
    AbPerpetualUserStreamDataSource,
)
from hummingbot.connector.derivative.ab_perpetual.data_sources.ab_perpetual_data_source import AbPerpetualClient
...
class AbPerpetualDerivative(PerpetualDerivativePyBase):
    web_utils = web_utils

    def __init__(
            self,
            client_config_map: "ClientConfigAdapter",
            ab_perpetual_secret_phrase: str,
            ab_perpetual_chain_address: str,
            trading_pairs: Optional[List[str]] = None,
            trading_required: bool = True,
            domain: str = CONSTANTS.DEFAULT_DOMAIN,
    ):
        self._ab_perpetual_secret_phrase = ab_perpetual_secret_phrase
        self._ab_perpetual_chain_address = ab_perpetual_chain_address
        self._trading_pairs = trading_pairs
        self._trading_required = trading_required
        self._domain = domain
        self._client_order_id_nonce_provider = NonceCreator.for_microseconds()

        self._tx_client: AbPerpetualClient = self._create_tx_client()
    ...
    async def _update_balances(self):
        path = f"{CONSTANTS.PATH_SUBACCOUNT}/{self._ab_perpetual_chain_address}/subaccountNumber/{self.subaccount_id}"
    ...
    async def _request_order_fills(self, order: InFlightOrder) -> Dict[str, Any]:
        ...
        body_params = {
            'address': self._ab_perpetual_chain_address,
    ...
    def _create_tx_client(self) -> AbPerpetualClient:
        return AbPerpetualClient(
            self._ab_perpetual_secret_phrase,
            self._ab_perpetual_chain_address,
            connector=self
        )

    def _create_order_book_data_source(self) -> AbPerpetualAPIOrderBookDataSource:
        return AbPerpetualAPIOrderBookDataSource(
            self.trading_pairs,
            connector=self,
    ...
    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return AbPerpetualUserStreamDataSource(api_factory=self._web_assistants_factory, connector=self)
